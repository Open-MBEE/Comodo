...
import EncoderHealthEstimator.EncoderHealthSV;
import MotorHealthEstimator.MotorHealthSV;
import NamedPositionEstimator.NamedPositionSV;
import NamedPositionController.Goal;
...
public abstract class SMActivity implements Runnable {
	...
	protected EncoderHealthSV mEncoderHealthSV = new EncoderHealthSV(); // subscribing
	protected MotorHealthSV mMotorHealthSV = new MotorHealthSV(); // subscribing
	protected NamedPositionSV mNamedPositionSV = new NamedPositionSV(); // subscribing
	protected Goal mGoal = new Goal(); // subscribing
	...
	public void openConnection() {
		try {
			...
			mChannel.queueBind(pubsubQueueName, DATA_EXCHANGE_NAME,
					"EncoderHealthSV");
			mChannel.queueBind(pubsubQueueName, DATA_EXCHANGE_NAME,
					"MotorHealthSV");
			mChannel.queueBind(pubsubQueueName, DATA_EXCHANGE_NAME,
	...
	public Message nextMessage(long timeout)
		...
		if (delivery != null) {
			...
			} else if (routingKey.matches("EncoderHealthSV")) {
				mEncoderHealthSV.setPayload(payload);
			} else if (routingKey.matches("MotorHealthSV")) {
				mMotorHealthSV.setPayload(payload);
			} else if (routingKey.matches("NamedPositionSV")) {
				mNamedPositionSV.setPayload(payload);
			} else if (routingKey.matches("Goal")) {
				mGoal.setPayload(payload);
			}
