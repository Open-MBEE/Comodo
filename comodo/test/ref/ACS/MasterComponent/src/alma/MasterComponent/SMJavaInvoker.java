...
public class SMJavaInvoker implements Invoker {
	...
	public void invoke(String source, Map params) throws InvokerException {
		...
		if (this.mSource.contentEquals("SMActivityinitSubsysPass1.java")) {
			myJob = new SMActivityinitSubsysPass1(parentStateId,
					parentSCInstance);
		} else

		if (this.mSource.contentEquals("SMActivityinitSubsysPass2.java")) {
			myJob = new SMActivityinitSubsysPass2(parentStateId,
					parentSCInstance);
		} else

		if (this.mSource.contentEquals("SMActivityreinitSubsystem.java")) {
			myJob = new SMActivityreinitSubsystem(parentStateId,
					parentSCInstance);
		} else

		if (this.mSource.contentEquals("SMActivityshutdownSubsysPass1.java")) {
			myJob = new SMActivityshutdownSubsysPass1(parentStateId,
					parentSCInstance);
		} else

		if (this.mSource.contentEquals("SMActivityshutdownSubsysPass2.java")) {
			myJob = new SMActivityshutdownSubsysPass2(parentStateId,
					parentSCInstance);
		} else
