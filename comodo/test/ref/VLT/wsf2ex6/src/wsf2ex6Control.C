...
/******************************************************************************
*   NAME
*   wsf2ex6Control - control control process
* 
*   SYNOPSIS
*   wsf2ex6Control [-r dbRoot] [-n processName] 
* 
*   DESCRIPTION
*
...
*
*   FILES
*   wsf2ex6_ERRORS - r - error message definitions
*
*   ENVIRONMENT
*
*   COMPILATION FLAGS
...
#include "wsf2ex6.h"
wsf2ex6WSF_RCSID("@(#) $Id: wsf2ex6Control.C 593 2012-04-22 12:12:12Z landolfa $")
...
int main(int argc, char *argv[])
{
    ...
    wsf2ex6APPLICATION app;
    if (wsf2libHELPER::ObjectOk(&app, "APPLICATION") == ccsFALSE)
		{
		wsf2libLOG_DEBUG() << "Cannot create application object";
		errAdd(wsf2ex6MOD, wsf2ex6ERR_FATAL, __FILE_LINE__, "cannot create APPLICATION object");
        errCloseStack();
        return EXIT_FAILURE;
		}
    if (app.Init(argc, argv) == FAILURE)
        ...
        errAdd(wsf2ex6MOD, wsf2ex6ERR_INIT, __FILE_LINE__, "cannot initialize the application");
        errCloseStack();
        return EXIT_FAILURE;
        } 
    ...
    if (app.Run() == FAILURE)
        ...
        errAdd(wsf2ex6MOD, wsf2ex6ERR_INIT, __FILE_LINE__, "cannot start the application");
        errCloseStack();
		return EXIT_FAILURE;
		} 
